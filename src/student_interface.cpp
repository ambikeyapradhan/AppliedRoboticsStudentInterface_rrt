...
//TODO: Change path
std::ofstream myfile("/home/ambikeya/workspace/project_3/AppliedRoboticsStudentInterface/src/results.txt");
//std::ofstream myfile("/home/robotics/workspace/group_5/src/results.txt");

// - - - - - - - - - - - - - - - 

void RRT(const float theta, Path& path, std::vector<Point>& biasPath, const Polygon& borders, int kmax, int npts, const std::vector<Polygon>& obstacle_list, std::vector<double> obs_radius, std::vector<Point> obs_center, double& length_path, const std::vector<double> gateInfo);
...
// - - - - - - - - -
  ...
  if (!std::experimental::filesystem::exists(extrinsic_path)){
    ...
    bool result = cv::solvePnP(object_points, image_points, camera_matrix, {}, rvec, tvec);
...
cv::Mat rotate(cv::Mat in_ROI, double ang_degrees){
  ...
  std::string template_folder = "/home/ambikeya/workspace/project_3/AppliedRoboticsStudentInterface/template/";
  ...
  bool detect_blue_robot(const cv::Mat& hsv_img, const double scale, Polygon& triangle, double& x, double& y, double& theta){
    ...
    // Bright w/ light
    cv::inRange(hsv_img, cv::Scalar(90, 20, 15), cv::Scalar(150, 255, 225), blue_mask);
    ...
    {
      // Approximate polygon w/ fewer vertices if not precise
      cv::approxPolyDP(contours[i], approx_curve, 30, true);	//TODO: change epsilon = 30
      std::cout << "Curve Size = " << approx_curve.size() << "\n";
      if (approx_curve.size() <= 2 || approx_curve.size() > 6) continue;
      robot_found = true;
      break;
    }
...
double getRandomSample(int MIN_R, int MAX_R){
  int range = rand()%(MAX_R-MIN_R+1) + MIN_R;
  double random = range/100.00;
  return random;
}

int findParentNode(std::vector <path_pos> &nodes_list, double x, double y, double &min_dist){
  int index = 0;
  for(int i=0; i<nodes_list.size(); i++){
    double sq_dist = pow((x-nodes_list.at(i).x), 2) + pow((y-nodes_list.at(i).y), 2);
...
void RRT(const float theta, Path& path, std::vector<Point>& biasPath, const Polygon& borders, int kmax, int num_pts, const std::vector<Polygon>& obstacle_list, std::vector<double> obs_radius, std::vector<Point> obs_center, double& length_path, const std::vector<double> gateInfo){
	//Initialise nodes and path variables
  ...
  path_pos first_node;
	...
	int MAX_X = (borders[1].x*100), MIN_X = (borders[0].x*100), MAX_Y = (borders[3].y*100), MIN_Y = (borders[0].y*100);
	int samp_X = 0, samp_Y = 0;
	double rand_x = 0, rand_y = 0;
	int rand_count = 1;
	
  //Initial Goal Index and flags
	int goal = 1; 
	bool failed_to_reach_goal = false;
	bool trying_for_goal = false;
	int wrong_initial_angles = 0;

	while(goal < biasPath.size()){
    //Clear last RRT	
		nodes_list.clear(), paths_list.clear();

		//If First Run of loop, Else we take last point in path as start
		if(goal == 1){
      first_node.x = biasPath[0].x;
      first_node.y = biasPath[0].y;
      ...
      first_node.theta = p.theta;
		...
		//Initialisation of RRT
		Path p = {};
		nodes_list.push_back(first_node);
		paths_list.push_back(p);
		bool goalReached = false;
		int loop_cnt = 1;
	
		while(goalReached == false){
      //Reset RRT if max iterations exceed or nodes exceed
      if(nodes_list.size() > max_nodes or loop_cnt == max_loops){
        if (loop_cnt == max_loops){
          ...
          if (mission_2 and wrong_initial_angles < 2){
            if (wrong_initial_angles == 0){
              nodes_list.at(0).theta = theta + 3.14/6.00;
            } 
            else {
              nodes_list.at(0).theta = theta - 3.14/6.00;
            }
        ...
        path_pos ni = nodes_list.at(0);
        Path pi = paths_list.at(0);
        nodes_list.clear(), nodes_list.push_back(ni);                          
        paths_list.clear(), paths_list.push_back(pi); 
      }
      ...
      while(!rand_clear){
        ...
        //Random Point is made goal every 5th iter
        if (rand_count % 5 == 0){
          rand_x =  biasPath[goal].x;
          rand_y =  biasPath[goal].y;
          trying_for_goal = true;
        }
        else{
          //sample random point
          rand_x = getRandomSample(MIN_X, MAX_X);
          rand_y = getRandomSample(MIN_Y, MAX_Y);
          rand_count += 1;
        }
        
        // Find closest node AKA parent
        index = findParentNode(nodes_list, rand_x, rand_y, min_dist);
        
        // Range of distance and goal check
        if((min_dist > 0.1 and min_dist < 0.4) || (rand_count % 5 == 0 && !failed_to_reach_goal) ){ //
          rand_clear = true;
        }
      }
      ...
      if (rand_x == biasPath[goal].x && rand_y == biasPath[goal].y && biasPath.size() != goal+1){
        ANGLE = compute_angle(Point(nodes_list.at(index).x,nodes_list.at(index).y), biasPath[goal+1]);
      // If point is gate, arrive orthogonally
      } else if(rand_x == gateInfo[0] && rand_y == gateInfo[1]){
        ANGLE = gateInfo[2];
      // If point before next goal or next goal is last
      } else {
        ANGLE = compute_angle(Point(nodes_list.at(index).x,nodes_list.at(index).y), biasPath[goal]);
      }
      
      Path newPath = {};
      dubinsCurve dubins = {};

      // Finding shortest path using dubins & discretizing the arcs
      dubins_shortest_path(dubins, nodes_list.at(index).x, nodes_list.at(index).y, nodes_list.at(index).theta, rand_x, rand_y, ANGLE, kmax); 
      discretize_arc(dubins.arc_1, s, num_pts, newPath); // Arc 1
      discretize_arc(dubins.arc_2, s, num_pts, newPath); // Arc 2
      discretize_arc(dubins.arc_3, s, num_pts, newPath); // Arc 3
      
      // Collision Checking and Check validity of new Path
      ...
      for(int j=0; j<newPath.points.size(); j++){
        ...
        if(newPath.points.at(j).x < (borders[0].x + 0.04) or newPath.points.at(j).x > (borders[1].x - 0.04) or newPath.points.at(j).y < (borders[0].y + 0.04)  or newPath.points.at(j).y > (borders[3].y - 0.04)){
          in_collision = true;
          if(trying_for_goal){
            failed_to_reach_goal = true; 
            trying_for_goal = false;
          }        
          break; 
        }

        // Checks if point is in collision
        for(int i=0; i<obstacle_list.size(); i++){
            double dist_to_ob = sqrt(pow((newPath.points.at(j).x-obs_center.at(i).x),2)+pow((newPath.points.at(j).y-obs_center.at(i).y),2));
            double result = insidePolygon(obstacle_list.at(i), Point(newPath.points.at(j).x,newPath.points.at(j).y)); // Possibly useless, since we check the radius of th
      ...
      if(in_collision == false){
        ...
        if(sqrt(pow((new_node.x - biasPath.at(goal).x),2)+pow((new_node.y - biasPath.at(goal).y),2)) < 0.1){
          goalReached = true;
          goal += 1;
        }
      }

      // if Goal Reached, backtrack the tree via parent indexes
      if(goalReached){
